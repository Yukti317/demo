...
import { Page, Layout, Grid, Text, TextField, Toast, RadioButton, Button, Checkbox, EmptySearchResult, IndexTable, Modal, Spinner, Icon, PageActions, Pagination, Badge, Card } from '@shopify/polaris';
...
const PercentageOff = () => {
    ...
    const validationSchema = Yup.object().shape({
        ...
        point_cost: Yup.number().nullable().min(1,"Point cost must be greater than 0").required('Points cost is required'),
        discount_value: Yup.number().nullable().min(1,"Discount valu must be greater than 0").required('Discount value is required'),
        min_purchase_amount: Yup.number().min(1,"Min purchase must be greater than 0").when('min_requirement', (min_requirement, field) => min_requirement[0] === '2' ? field.required('Min purchase amount is required') : field),
        expire_day: Yup.number().nullable().required('expiry days is required').min(1,"Expiry day must be greater than 0"),
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            ...
            if (!id) {
                const res = await ApiCall('POST', '/percentage_off', data, { authentication: token });
                ...
                data.id = parseInt(id);
                const res = await ApiCall('PUT', '/update_percentage_off', data, { authentication: token });
                if (res.data.status === 'SUCCESS' && res.status === 200) {
                    setSaveActiveToastMessage(res.data.message);
                    toggleActive();
                    setTimeout(() => {
    ...
    const GetAmountData = async () => {
        setLoading(true);
        const res = await GetApiCall('GET', `/percentageOff_get/${id}`, { authentication: token });
        if (res.data.status === "SUCCESS" && res.status === 200) {
            ...
            data.discount_value = detailData.discount_value
    ...
    const deleteReward = async (id) => {
        ...
        const res = await ApiCall('DELETE', '/delete_percentage_off', { id: parseInt(id) }, { authentication: token });
        if (res.data.status === 'SUCCESS' && res.status === 200) {
            setTimeout(() => {
                navigate('/redeem-points');
            }, 1000)
    ...
    return (
        <>
            {process.env?.MODE !== 'local' && !config_variable?.shop_url.includes('aapharmacy.com.my') ? <Provider config={config_variable.config}>
                <TitleBar title='Percentage off' breadcrumbs={redeemPointsBreadcrumb} />
            </Provider> : <></>}
            {!loading ? <div className='fix-amount-discount-page amount-pages'><Page title='Percentage off' backAction={{ content: 'Products', onAction: () => navigate('/redeem-points') }} primaryAction={{ primary: true, content: 'Save', loading: saveloading, disabled: !formik.dirty, onAction: formik.handleSubmit }}>
                <Layout>
                    <Layout.Section>
                        <div className='pb-3'>
                            <Card>
                                    ...
                                    <Grid.Cell columnSpan={{ xs: 6, sm: 3, md: 3, lg: 6, xl: 6 }}>
                                        <div className=''>
                                            <TextField
                                                ...
                                                onChange={(value) => { formik.setFieldValue('discount_value', value) }}
                                                onBlur={() => formik.setFieldTouched('discount_value')}
                                                error={formik.touched.discount_value && formik.errors.discount_value ? formik.errors.discount_value : ''}
                                                autoComplete="off"
                                            />
                                        </div>
                                    </Grid.Cell>
