...
import { Page, Layout, Grid, Text, Button, Spinner, Toast, TextField, EmptySearchResult, IndexTable, Checkbox, Modal, Icon, RadioButton, Pagination, PageActions, Badge, Card } from '@shopify/polaris'
...
const FixedAmountDiscount = () => {
    ...
    const validationSchema = Yup.object().shape({
        ...
        discount_value: Yup.number().nullable().min(1,"Discount value must be greater than 0").required('Discount value is required'),
        ...
        expire_day: Yup.number().nullable().required('Reward value days is required').min(1, "Expiry day must be greater than 0"),
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            ...
            if (!id) {
                const res = await ApiCall('POST', '/fixed_amount_discount', data, { authentication: token });
                ...
                data.id = parseInt(id);
                const res = await ApiCall('PUT', '/update_fixed_amount_discount', data, { authentication: token });
                if (res.data.status === 'SUCCESS' && res.status === 200) {
                    setSaveActiveToastMessage(res.data.message);
                    toggleActive();
                    setTimeout(() => {
    ...
    const GetAmountData = async () => {
        setLoading(true);
        const res = await GetApiCall('GET', `/Amount_Discount_get/${id}`, { authentication: token });
        if (res.data.status === "SUCCESS" && res.status === 200) {
    ...
    const deleteReward = async (id) => {
        ...
        const res = await ApiCall('DELETE', '/delete_fixed_amount_discount', { id: parseInt(id) }, { authentication: token });
        if (res.data.status === 'SUCCESS' && res.status === 200) {
            setSaveActiveToastMessage(res.data.message);
            toggleActive();
            setTimeout(() => {
    ...
    return (
        <>
            {process.env?.MODE !== 'local' && !config_variable?.shop_url.includes('aapharmacy.com.my') ? <Provider config={config_variable.config}>
                <TitleBar title='Fixed Amount Discount' breadcrumbs={redeemPointsBreadcrumb} />
            </Provider> : <></>}
            {!loading ? <div className='fix-amount-discount-page amount-pages'>
                <Page title='Fixed Amount Discount' backAction={{ content: 'Products', onAction: () => navigate('/redeem-points') }} primaryAction={{ primary: true, disabled: !formik.dirty, content: 'Save', loading: saveloading, onAction: formik.handleSubmit }}>
                    <Layout>
                        <Layout.Section>
                            <div className='pb-3'>
                                <Card>
                    ...
                    <div className="collectionmodal">
                        <Modal
                            ...
                            footer={
                                (!datacollectionState.collectionData.length && (formik.values.collectionState.collectionSearchValue === '' || formik.values.collectionState.collectionSearchValue?.length >= 0)) ? (<></>) : (
                                    <div>
